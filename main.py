...
parser.add_argument('--agg_hidden', type=int, default=128)
parser.add_argument('--fc_hidden', type=int, default=32)
parser.add_argument('--agg_method', type=str, default="sum")
...
model = GraphSAGE(n_layer=args.n_layer, agg_hidden=args.agg_hidden, fc_hidden=args.fc_hidden).to(device)
...
def predict_test(model, test_loader, file_name):
    ...
    for i, graph in enumerate(iter(test_loader)):
        ...
        predicted_sureness = torch.max(probs)

        prediction_df.loc[i] = [predicted_class.item(), predicted_sureness.item()]
...
predict_test(model, test_loader, file_name="predictions.csv")
