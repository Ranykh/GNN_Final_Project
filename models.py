...
class GAT(torch.nn.Module):
    def __init__(self, n_layer, agg_hidden, fc_hidden, in_channels=7, num_classes=2, agg_method="avg", dropout=0.4):
...
class GraphSAGE(torch.nn.Module):
    def __init__(self, n_layer, agg_hidden, fc_hidden, in_channels=7, num_classes=2, agg_method="avg", dropout=0.3):
...
class GaussianGraphSAGE(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        log_var = F.relu(self.var_conv[0](x, edge_index,edge_attr))
        
        for i in range(1, len(self.mean_conv)):
            mean = F.relu(self.mean_conv[i](mean, edge_index, edge_attr))
            mean = F.dropout(mean, p=self.dropout, training=self.training)
            log_var = F.relu(self.var_conv[i](log_var, edge_index, edge_attr))
